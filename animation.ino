 ...
 */
typedef uint16_t (*AnimationStepFunction)(uint8_t);

uint16_t fadeGreen(uint8_t step);
uint16_t fadeRed(uint8_t step);
...
const AnimationSpec_t animation1 = { 2, 0, { .steps = animation1_Steps } };
...
const AnimationSpec_t animation2 = { 2, 0, { .steps = animation2_Steps } };

const AnimationSpec_t animation3 = { 16, 1, { .fx = fadeGreen } };
const AnimationSpec_t animation4 = { 16, 1, { .fx = fadeRed } };
...
void animationStep()
{
  if (millis()> nextStepChange)
  {
    ...
    {
      nextStepChange = millis() + currentAnimation->fx(currentStep);
    }
...
uint16_t fadeRed(uint8_t step)
{
  ...
  if (step < 8)
  {
    color = ((uint32_t)0x20 * (8 - step) - 1) << 16;
  }
  else
  {
    color = ((uint32_t)0x20 * (step - 8)) << 16;
  }
